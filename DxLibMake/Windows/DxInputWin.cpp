...
BOOL FAR PASCAL EnumJoypadProc( const D_DIDEVICEINSTANCEW *pdinst , LPVOID /*pvRef*/ )
{
	...
	// 情報表示
	{
		...
		_WCSCPY( pad->PF.InstanceName, State.tszInstanceName );
		_WCSCPY( pad->PF.ProductName,  State.tszProductName  );
...
MOUSEDEVICEINITEND:
	...
	//		hr = InputSysData.PF.KeyboardDeviceObject->SetDataFormat( &c_dfDIKeyboard ) ;
			...
			// キーボードの協調レベルを設定する
			InputSysData.PF.KeyExclusiveCooperativeLevelFlag = FALSE ;
...
//			pad->EffectPlayStateRight	= FALSE ;
			...
			// 名前を設定
			_WCSCPY( pad->PF.InstanceName, L"Controller (XBOX 360 For Windows)" );
			_WCSCPY( pad->PF.ProductName,  L"Controller (XBOX 360 For Windows)" );
...
extern int UpdateKeyboardInputState_PF( int UseProcessMessage )
{
	...
	if( InputSysData.PF.NoUseDirectInputFlag == TRUE || InputSysData.PF.KeyboardNotUseDirectInputFlag == TRUE || InputSysData.PF.KeyboardDeviceObject == NULL || ( InputSysData.PF.UseDirectInputFlag == FALSE && WinData.WindowsVersion >= DX_WINDOWSVERSION_VISTA ) )
	{
