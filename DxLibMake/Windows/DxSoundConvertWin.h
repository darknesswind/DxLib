...
#ifndef DX_NON_ACM
extern	int SetupSoundConvert_ACM(            SOUNDCONV *SoundConv ) ;								// ＡＣＭを使用したファイルのセットアップ処理を行う( [戻] -1:エラー )
extern	int SetupSoundConvert_MP3(            SOUNDCONV *SoundConv ) ;								// ＭＰ３ファイルのセットアップ処理を行う( [戻] -1:エラー )
extern	int TerminateSoundConvert_ACM(        SOUNDCONV *SoundConv ) ;								// ＡＣＭを使用したファイルの後始末処理を行う
extern	int ConvertProcessSoundConvert_ACM(   SOUNDCONV *SoundConv ) ;								// 変換後のバッファにデータを補充する
//extern	int SetTimeSoundConvert_ACM(      SOUNDCONV *SoundConv, int Time ) ;					// 変換処理を位置を変更する( ミリ秒単位 )
extern	int SetSampleTimeSoundConvert_ACM(    SOUNDCONV *SoundConv, int SampleTime ) ;				// 変換処理の位置を変更する( サンプル単位 )
extern	int GetSoundConvertDestSize_Fast_ACM( SOUNDCONV *SoundConv ) ;								// 変換後の大凡のデータサイズを得る
#endif

#ifndef DX_NON_MOVIE
#ifndef DX_NON_DSHOW_MOVIE
#ifndef DX_NON_DSHOW_MP3
extern	int TerminateSoundConvert_DSMP3(        SOUNDCONV *SoundConv ) ;							// MP3を使用したファイルの後始末処理を行う
extern	int ConvertProcessSoundConvert_DSMP3(   SOUNDCONV *SoundConv ) ;							// 変換後のバッファにデータを補充する
//extern	int SetTimeSoundConvert_DSMP3(      SOUNDCONV *SoundConv, int Time ) ;					// 変換処理を位置を変更する( ミリ秒単位 )
extern	int SetSampleTimeSoundConvert_DSMP3(    SOUNDCONV *SoundConv, int SampleTime ) ;			// 変換処理の位置を変更する( サンプル単位 )
extern	int GetSoundConvertDestSize_Fast_DSMP3( SOUNDCONV *SoundConv ) ;							// 変換後の大凡のデータサイズを得る
#endif
#endif
#endif
