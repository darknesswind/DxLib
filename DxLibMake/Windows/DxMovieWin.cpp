...
)
{
	...
	if( FAILED( Movie->PF.pGraph->QueryInterface( IID_IMEDIASEEKING, ( void ** )&Movie->PF.pMediaSeeking ) ) )
	{
   	...
   	// ファイル名保存
	_WCSCPY( Movie->PF.FileName, FileName ) ;
	...
	if( FAILED( hr ) )
	{
		...
		// 開けた場合はテンポラリファイルに書き出す
		{
			...
			// テンポラリファイルの作成
			FileHandle = CreateTemporaryFile( Movie->PF.FileName ) ;
...
extern	BASEIMAGE *GetMovieBaseImage_PF( MOVIEGRAPH * Movie, int *ImageUpdateFlag )
{
#ifndef DX_NON_DSHOW_MOVIE
	// ムービーのフレームを更新
