...
HRESULT D_CBaseVideoRenderer::ResetStreamingTimes()
{
	...
	m_tStreamingStart = WinAPIData.Win32Func.timeGetTimeFunc();
	m_trRenderAvg = 0;
...
{
	m_tStreamingStart = WinAPIData.Win32Func.timeGetTimeFunc()-m_tStreamingStart;
	return NOERROR;
} // OnStopStreaming
...
{
	RecordFrameLateness(m_trLate, m_trFrame);
	m_tRenderStart = WinAPIData.Win32Func.timeGetTimeFunc();
} // OnRenderStart
...
void D_CBaseVideoRenderer::OnRenderEnd(D_IMediaSample * /*pMediaSample*/)
{
	...
	int tr = (WinAPIData.Win32Func.timeGetTimeFunc() - m_tRenderStart)*10000;	// convert mSec->UNITS
...
STDMETHODIMP D_CBaseVideoRenderer::get_AvgFrameRate( int *piAvgFrameRate)
{
	...
	int t;
	if (m_bStreaming) {
		t = WinAPIData.Win32Func.timeGetTimeFunc()-m_tStreamingStart;
	} else {
		t = m_tStreamingStart;
	}
...
D_CMediaType::ReallocFormatBuffer(ULONG length)
{
	...
	if (cbFormat != 0) {
		ASSERT(pbFormat);
		_MEMCPY(pNewFormat,pbFormat,min(length,cbFormat));
		WinAPIData.Win32Func.CoTaskMemFreeFunc((PVOID)pbFormat);
	}
...
void D_CMovieRender::AllocImageBuffer()
{
	...
	{
		TempBaseImage = new DxLib::BASEIMAGE ;
		NS_CreateXRGB8ColorBaseImage( Width, Height, TempBaseImage ) ;
	}
...
HRESULT D_CMovieRender::DoRenderSample( D_IMediaSample * pSample )
{
	...
	if( ImageBuffer )
	{
		...
		if( UseTempBaseImage != 0 )
		{
			...
			{
			case 2 :	// YV12
				ConvertYV12ToXRGB32( Src, Width, Height, TempBaseImage ) ;
				break ;

			case 3 :	// NV12
				ConvertNV12ToXRGB32( Src, Width, Height, TempBaseImage ) ;
				break ;

			case 4 :	// YUY2
				ConvertYUY2ToXRGB32( Src, Width, Height, TempBaseImage ) ;
				break ;

			case 5 :	// UYVY
				ConvertUYVYToXRGB32( Src, Width, Height, TempBaseImage ) ;
				break ;

			case 6 :	// UYVY
				ConvertYVYUToXRGB32( Src, Width, Height, TempBaseImage ) ;
				break ;

			case 7 :	// NV11
				ConvertNV11ToXRGB32( Src, Width, Height, TempBaseImage ) ;
				break ;
			}
...
)
{
    if (0 != cbProperties) {
        ...
        Props.cbData     = (DWORD) (min(cbProperties, ( DWORD )sizeof(Props)));
